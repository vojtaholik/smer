...
- **Explicit export.** Pages are generated locally. Serving or publishing requires a separate user-invoked action and an explicit destination; smer itself never uploads anything.
...
This remains deferred until M1 proves value and a dedicated privacy review passes.
...
Potentially extract the search query from search-engine URLs the browser provider already harvests. This is deferred behind an explicit opt-in and privacy review because queries can be substantially more sensitive than domains and page titles.

Deterministic, no new sources, and the existing domain denylist is untouched.

### Vercel Failure Context

Potentially fetch a bounded, redacted tail when a deployment ends in error. This is deferred with stderr capture until M1 proves value and the storage and API-cost budgets are verified.
...
Its versioned JSON contract uses equal half-open windows and bounded, stable-sorted sections.
...
- **Outcome evidence** — deploys, failed jobs, and future outcome-provider events with bounded verbatim excerpts because even rare event text can be large.
...
Distillations are corpus events: `x-observation`, `x-open-loop`, and `x-decision` kinds emitted via `smer emit --spool`, each citing the event ids that support it plus a schema version and producer identity.

Brief generation excludes these derived kinds so observations cannot recursively amplify themselves.
...
The renderer is deliberately dumb — it queries recent `x-observation` and `x-open-loop` events and formats them; all judgment already happened in the sweep. Output is self-contained HTML under `~/.smer` with private permissions. Serving or sharing is a separate, explicit user action with a named destination; smer never publishes on its own.
...
4. **M4 — Outcome capture.** GitHub waiting-on-me and provider hints first; stderr tails, Vercel log excerpts, and browser queries require separate opt-ins, privacy review, and budget evidence.
